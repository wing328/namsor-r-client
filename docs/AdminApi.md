...
#Print historical API usage.
result = AdminApi$ApiUsageHistory()
dput(result)
```
...
#Print historical API usage (in an aggregated view, by service, by day/hour/min).
result = AdminApi$ApiUsageHistoryAggregate()
dput(result)
```
...
#List all available plans in the default currency (usd).
result = AdminApi$AvailablePlans()
dput(result)
```
...
#List all available plans in the user's preferred currency.
result = AdminApi$AvailablePlans1(var.token)
dput(result)
```
...
#List of API services and usage cost in Units (default is 1=ONE Unit).
result = AdminApi$AvailableServices()
dput(result)
```
...
#List possible currency options for billing (USD, EUR, GBP, ...)
result = AdminApi$BillingCurrencies()
dput(result)
```
...
#Read the history billing information (invoices paid via Stripe or manually).
result = AdminApi$BillingHistory(var.token)
dput(result)
```
...
#Read the billing information (company name, address, phone, vat ID)
result = AdminApi$BillingInfo(var.token)
dput(result)
```
...
#Create a Stripe Customer, based on a payment card token (from secure StripeJS) and email.
result = AdminApi$Charge(inline.object=var.inline.object)
dput(result)
```
...
#Setting an API Key to a corporate status.
AdminApi$CorporateKey(var.api.key, var.corporate)
```
...
#Update debug level for a classifier
AdminApi$DebugLevel(var.logger, var.level)
```
...
#Get the Stripe payment information associated with the current google auth session token.
result = AdminApi$PaymentInfo(var.token)
dput(result)
```
...
#Procure an API Key (sent via Email), based on an auth token. Keep your API Key secret.
result = AdminApi$ProcureKey(var.token)
dput(result)
```
...
#Subscribe to a give API plan, using the user's preferred or default currency.
result = AdminApi$RemoveUserAccount(var.token)
dput(result)
```
...
#Get the current software version
result = AdminApi$SoftwareVersion()
dput(result)
```
...
#Print basic source statistics.
result = AdminApi$SourceStats(var.source)
dput(result)
```
...
#Connects a Stripe Account.
AdminApi$StripeConnect(scope=var.scope, code=var.code, error=var.error, error.description=var.error.description)
```
...
#Subscribe to a give API plan, using the user's preferred or default currency.
result = AdminApi$SubscribePlan(var.plan.name, var.token)
dput(result)
```
...
#Sets or update the billing information (company name, address, phone, vat ID)
result = AdminApi$UpdateBillingInfo(var.token, billing.info.in.out=var.billing.info.in.out)
dput(result)
```
...
#Modifies the hard/soft limit on the API plan's overages (default is 0$ soft limit).
result = AdminApi$UpdateLimit(var.usage.limit, var.hard.or.soft, var.token)
dput(result)
```
...
#Update the default Stripe card associated with the current google auth session token.
result = AdminApi$UpdatePaymentDefault(var.defaut.source.id, var.token)
dput(result)
```
...
#Get the user profile associated with the current google auth session token.
result = AdminApi$UserInfo(var.token)
dput(result)
```
...
#Verifies an email, based on token sent to that email
result = AdminApi$VerifyEmail(var.email.token)
dput(result)
```
...
#Verifies an email, based on token sent to that email
result = AdminApi$VerifyRemoveEmail(var.email.token)
dput(result)
```
...
#Vetting of a source.
AdminApi$Vet(var.source, var.vetted)
```
